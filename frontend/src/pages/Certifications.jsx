...
const EMPTY = { title: '', organization: '', year: '', link: '' }
...
export default function Certifications() {
  ...
  const openEdit = (entry) => { setForm({ title: entry.title||'', organization: entry.organization||'', year: entry.year||'', link: entry.link||'' }); setEditId(entry.id); setErrors({}); setShowForm(true) }
  ...
  return (
    <div>
      <div className="page-header"><h2><span className="header-icon">🏅</span> Certifications</h2>{!showForm && <button className="btn btn-primary" onClick={openAdd}>+ Add Certification</button>}</div>
      <MessageAlert message={msg.text} type={msg.type} onClose={() => setMsg({ text: '', type: '' })} />
      {showForm && (
        <div className="form-section">
          <h3>📝 {editId ? 'Edit' : 'Add'} Certification</h3>
          <form onSubmit={handleSubmit}>
            <div className="form-row">
              <div className="form-group"><label>Title *</label><input type="text" name="title" value={form.title} onChange={handleChange} placeholder="AWS Solutions Architect" required />{errors.title && <div className="field-error">{errors.title}</div>}</div>
              <div className="form-group"><label>Organization *</label><input type="text" name="organization" value={form.organization} onChange={handleChange} placeholder="Amazon" required />{errors.organization && <div className="field-error">{errors.organization}</div>}</div>
            </div>
            <div className="form-row">
              ...
              <div className="form-group"><label>Link</label><input type="url" name="link" value={form.link} onChange={handleChange} placeholder="https://credential.net/..." />{errors.link && <div className="field-error">{errors.link}</div>}</div>
            </div>
      ...
      {entries.length === 0 && !showForm ? (
        <div className="empty-state"><div className="empty-icon">🏅</div><h3>No Certifications Yet</h3><p>Add certifications.</p><button className="btn btn-primary" onClick={openAdd}>+ Add</button></div>
      ) : (
        <div className="entry-list">
          {entries.map((e) => (
            <div key={e.id} className="entry-card">
              <div className="entry-info"><h4>{e.title}</h4><div className="entry-subtitle">{e.organization}</div><div className="entry-date">📅 {e.year}</div>{e.link && <a href={e.link} target="_blank" rel="noopener noreferrer" className="entry-link">🔗 View</a>}</div>
